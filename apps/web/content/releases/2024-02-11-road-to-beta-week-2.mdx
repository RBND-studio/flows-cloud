...
**Flow error tracking** - see where flows fail and fix them. Error events shows you where exactly the flow failed and on which element.

**Async load from Cloud** - to speed up the initialization even more, Flows now load only the first step of each flow the user can see and load the rest only when a flow starts.
