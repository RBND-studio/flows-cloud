...
Out of the box, Flows supports dark mode using the class `dark-mode` on the `body` element. You can change the selector to match your theming implementation. After you have set up the selector, copy the color variables from the default theme and change their values to match your dark theme.
...
Customizing the full CSS template requires a good understanding of CSS and the structure of the onboarding flows. We recommend using the CSS variables template whenever possible. Make sure to preview and test your changes in the onboarding flows to ensure they look good on all devices and screen sizes.
