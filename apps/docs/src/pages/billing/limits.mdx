...
This happens because Flows SDK loads all flows the user can see at once, and the launches them when conditions are met. Which means that if a user is initialized before the limit is reached, they'll still be able to see the flows even if the limit is reached later. If you feel like you overshot the limit by too much, please contact us and we'll help you out.
